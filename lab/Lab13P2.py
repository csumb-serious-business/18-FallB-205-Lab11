...
class Player(object):
    ...
    M_NEW_NAME = "You shall be known hitherto as '%s'."
...
class GameEnvironment(object):
    ...
    def __init__(self):
        ...
        # setup the triggers
        self.add_trigger(t_d.can_be_target_of, self.win_game, None, Commands.LOCK)
        self.add_trigger(t_c.cannot_be_target_of, self.lose_game, None, Commands.THROW)
    ...
    def set_player_name(self, name):
        """
        sets the player name for win/loss messages
          if the new name is empty, it will remain the initial 'Mysterious denizen'
        :param name: the player's name
        :return: a Tuple of Boolean, String
           the bool: True if the new name was not empty
           the string: the message to show the user
        """
        ...
        if len(name) > 0:
            ...
            success = True
        return success, Player.M_NEW_NAME % self.player.name
...
class Commands(object):
    ...
    """
    HELP, EXIT, NAME_PLAYER, NORTH, SOUTH, EAST, WEST, TAKE, THROW, LOCK, UNLOCK = range(11)
...
class Adventure(object):
    ...
    M_COMMAND_ERROR = "Something has gone awry when attempting" \
                      " to '%s', but what?"
    M_WIN = "Congratulations %s, You win!"
    M_LOSE = "Unfortunately that was a poor choice %s, You lose."
    ...
    def play(self):
        ...
        player_name = self.ui.ask("what shall we call you, adventurer?")
        set_result = self.environment.set_player_name(player_name)
        self.ui.show(set_result[1])
        ...
        if self.environment.game_is_won:
            self.ui.show(self.M_WIN % self.environment.player.name)
        else:
            self.ui.show(self.M_LOSE % self.environment.player.name)
    ...
    def execute_command(self, command, param):
        ...
        if command == Commands.HELP:
            return self.do_help()
        # not a user-accessible command
        # if command == Commands.NAME_PLAYER:
        #     return self.environment.set_player_name(param)
...
def main():
    ...
    :return: None
    """
    Log.enabled = True  # False to disable log messages
